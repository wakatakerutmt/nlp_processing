...
def squad_convert_example_to_features(example, max_seq_length, doc_stride, max_query_length, is_training):
    ...
    if is_training and not example.is_impossible:
        ...
        # cleaned_answer_text = " ".join(tokenizer.tokenize(example.answer_text))
...
class SquadProcessor(DataProcessor):
    ...
    def _get_example_from_tensor_dict(self, tensor_dict, tokenizer, evaluate=False):
        if not evaluate:
    ...
    def get_examples_from_dataset(self, dataset, tokenizer, evaluate=False):
        """
        ...
        for tensor_dict in tqdm(dataset):
            examples.append(self._get_example_from_tensor_dict(
                tensor_dict, tokenizer, evaluate=evaluate))

        return examples

    def get_train_examples(self, data_dir, tokenizer, filename=None):
        """
    ...
    def get_dev_examples(self, data_dir, tokenizer, filename=None):
        """
...
class SquadExample(object):
    ...
    def __init__(
        ...
        if start_position_character is not None and not is_impossible:
            ...
            ans_tokens = tokenizer.tokenize(answer_text)
            if ans_tokens[0] == '▁':
                ans_tokens = ans_tokens[1:]
            else:
                ans_tokens[0] = ans_tokens[0].replace('▁', '', 1)

            start_position = 0
            end_position = 0
            for idx_start in range(len(doc_tokens)):
                match = True

                if doc_tokens[idx_start] == ans_tokens[0]:
                    idx_end = idx_start

                    for ans_token in ans_tokens:
                        if doc_tokens[idx_end] != ans_token:
                            match = False
                            break
                        idx_end += 1

                    if match == True:
                        start_position = idx_start
                        end_position = idx_end - 1
