...
def normalize_answer(s):
    ...
    def lower(text):
        ...
        # , と . と（ と ）の全角はsentencepieceでは半角に変換されてしまうのでそれに対応する
        text = text.replace('，', ',')
        text = text.replace('．', '.')
        text = text.replace('（', '(')
        text = text.replace('）', ')')
...
def get_raw_scores(examples, preds, tokenizer):
    """
    ...
    for example in examples:
        ...
        f1_scores[qas_id] = max(compute_f1(a, prediction, tokenizer)
...
def squad_evaluate(examples, preds, tokenizer, no_answer_probs=None, no_answer_probability_threshold=1.0):
    ...
    exact, f1 = get_raw_scores(examples, preds, tokenizer)
