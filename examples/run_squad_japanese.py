...
from transformers.data.processors.squad_japanese import SquadV1Processor, SquadV2Processor, SquadResult
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache and not output_examples:
        ...
        if not args.data_dir and ((evaluate and not args.predict_file) or (not evaluate and not args.train_file)):
            ...
            examples = SquadV1Processor().get_examples_from_dataset(
                tfds_examples, tokenizer, evaluate=evaluate)
        else:
            processor = SquadV2Processor() if args.version_2_with_negative else SquadV1Processor()

            if evaluate:
                examples = processor.get_dev_examples(
                    args.data_dir, tokenizer, filename=args.predict_file)
            else:
                examples = processor.get_train_examples(
                    args.data_dir, tokenizer, filename=args.train_file)
